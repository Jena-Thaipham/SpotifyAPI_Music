...
WITH artist_stats AS (
    SELECT 
        ...
        COUNT(DISTINCT al.album_id) AS album_count,
        COUNT(DISTINCT pt.playlist_id) AS playlist_count,
        ROUND(AVG(COALESCE(t.popularity, 0)),2) AS track_popularity,
        ROUND(AVG(COALESCE(al.popularity, 0)),2) AS album_popularity
    FROM artists ar
    LEFT JOIN tracks t ON ar.artist_id = t.artist_id
    LEFT JOIN albums al ON ar.artist_id = al.artist_id
    LEFT JOIN playlist_tracks pt ON t.track_id = pt.track_id
...
artist_with_flags AS (
    SELECT 
        ...
        CASE WHEN a.track_popularity > avg.avg_track_popularity THEN 'Yes' ELSE 'No' END AS top_by_track_rate,
        CASE WHEN a.album_popularity > avg.avg_album_popularity THEN 'Yes' ELSE 'No' END AS top_by_album_rate,
...
-- Album By Year of Top Emerging Artists By Count
WITH emerging_artists AS (
    SELECT 
        ar.artist_id,
        ar.artist_name,
...
marked_artists AS (
    SELECT 
        ...
        ea.playlist_count,
        CASE 
            WHEN ea.followers > avg.avg_followers
              AND ea.track_count > avg.avg_track_count
              AND ea.album_count > avg.avg_album_count
              AND ea.playlist_count > avg.avg_playlist_count
            THEN 'Yes' ELSE 'No'
        END AS is_top_artist
    FROM emerging_artists ea
    CROSS JOIN emerging_avg avg
)
...
SELECT 
    ...
    ma.is_top_artist,
    COUNT(DISTINCT al.album_id) AS album_count
FROM marked_artists ma
JOIN albums al ON ma.artist_id = al.artist_id
GROUP BY ma.artist_name, release_year, ma.is_top_artist
ORDER BY ma.is_top_artist DESC, ma.artist_name, release_year;
...
tracks_with_popularity AS (
    SELECT 
        ...
        t.track_name,
        t.popularity,
        ROW_NUMBER() OVER (PARTITION BY ar.artist_id ORDER BY t.popularity DESC) AS rn
